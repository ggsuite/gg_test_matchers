...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_test_matchers.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggTestMatchers = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_test_matchers.dart',
      );

      expect(
        ggTestMatchers,
        isNotNull,
        reason: 'Item for executing bin/gg_test_matchers.dart not found',
      );

      expect(ggTestMatchers['name'], 'gg_test_matchers.dart');
      expect(ggTestMatchers['type'], 'dart');
      expect(ggTestMatchers['request'], 'launch');
